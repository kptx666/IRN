...
parser.add_argument("--checkpoint", type=str, default='checkpoint_x2/best_epoch_849.pth',
...
model = ConvNeXt(upscale_factor=opt.upscale_factor)
...
#     print(iname)
#     tests_name = opt.test_lr_folder + iname.split('/')[-1].split('.')[0]+ext
#     print(tests_name)
for imname in filelist:
    im_gt = cv2.imread(imname, cv2.IMREAD_COLOR)[:, :, [2, 1, 0]]  # BGR to RGB
    im_gt = utils.modcrop(im_gt, opt.upscale_factor)
    im_l = cv2.imread(opt.test_lr_folder + imname.split('/')[-1].split('.')[0] + ext,
